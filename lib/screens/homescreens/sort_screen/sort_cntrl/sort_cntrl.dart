...
import '../sort_api/sort_model.dart';
...
class SortController extends GetxController {
  ...
  RxString selectedProductId =
      '3'.obs; // Use RxString instead of RxList<String>
  ...
  void handleProductSelection() {
    for (int index = 0; index < checkboxValues.length; index++) {
      if (checkboxValues[index]) {
        String? productId = categories.value.category![index].categoryId;
        selectedProductId.value = productId ?? '';
      }
    }
    // Clear the productsdata list when filtering changes
    productsdata.clear();
    fetchProducts();
